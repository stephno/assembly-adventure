; Aliases for the Memory Mapped Registers to be used.
INIDISP     = $2100 ; initial settings for screen.
OBJSEL      = $2101 ; object size $ object area designation.
...
.proc   ResetHandler
        ...
        ldx #$00        ; X will serve as a loop counter and offset.
VRAMLoop:
        ...
        inx
        cpx #$80            ; Have we written $04*$20 = $80 bytes
                            ; to VRAM (4 sprites)?
        ...
        ldx #$00            ; X = counter loop and offset.
CGRAMLoop:
        ...
        lda # (256/2 - 8)   ; horizontal position of the first sprite.
        sta OAMDATA
        lda # (224/2 - 8)   ; vertical position of the first sprite.
        sta OAMDATA
        lda #$00            ; name of the first sprite.
        sta OAMDATA
        lda #$00            ; no flip, prio 0, palette 0
        sta OAMDATA
        ; OAM data for second sprite
        lda # (256/2)       ; horizontal position of the second sprite.
        sta OAMDATA
        lda # (224/2 - 8)   ; vertical position of the second sprite.
        sta OAMDATA
        lda #$01            ; name of the second sprite.
        sta OAMDATA
        lda #$00            ; no flip, prio 0, palette 0.
        sta OAMDATA
        ; OAM data for the third sprite
        lda # (256/2 - 8)   ; horizontal position of the third sprite.
        sta OAMDATA
        lda # (224/2)       ; vertical position of the third sprite.
        sta OAMDATA
        lda #$02            ; name of the third sprite.
        sta OAMDATA
        lda #$00            ; no flip, prio 0, palette 0.
        sta OAMDATA
        ; OAM data for the fourth sprite
        lda # (256/2)       ; horizontal position of the fourth sprite.
        sta OAMDATA
        lda # (224/2)       ; vertical position of the fourth sprite.
        sta OAMDATA
        lda #$03            ; name of the fourth sprite.
        sta OAMDATA
        lda #$00            ; no flip, prio 0, palette 0.
        sta OAMDATA
